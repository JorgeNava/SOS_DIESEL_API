...
async function updateProduct(productParams) {
	try {
		...
		if (productParams.quantity)
			fieldsToUpdate.Quantity = productParams.quantity;
		if (productParams.name) fieldsToUpdate.Name = productParams.name;
		if (productParams.description)
			fieldsToUpdate.Description = productParams.description;
		if (productParams.brand) fieldsToUpdate.Brand = productParams.brand;
		if (productParams.truckModel)
			fieldsToUpdate.TruckModel = productParams.truckModel;
		...
		const updatedProduct = await CatalogTable.update(
			product.id,
			fieldsToUpdate
		);
